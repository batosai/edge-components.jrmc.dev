      ...
      ctx.view.share({
        ...
        fakeUsers: ({ currentPage, total }: any) => {
          ...
          return {
            ...
            getUrlsForRange: (first: string, last: string) =>
              urls.filter((url: any) => url.page >= first && url.page <= last),
        ...
        appVersion: () =>
          JSON.parse(
            fs
              .readFileSync(app.makePath(`./node_modules/@jrmc/edge-components/package.json`))
              .toString()
          ).version,
      })
    }
